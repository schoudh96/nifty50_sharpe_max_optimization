...
def html2(returns, benchmark=None, rf=0.,
         grayscale=False, title='Strategy Tearsheet',
         output=None, mode = 'full', compounded=True):
...
def html_original(returns, benchmark=None, rf=0.,
         grayscale=False, title='Strategy Tearsheet',
         output=None, mode = 'full', compounded=True):
...
def html(returns, benchmark=None, rf=0., rf_rate = 0,
         corr = None, turnover = None, ic = None,
         grayscale=False, title='Strategy Tearsheet',
         output=None, compounded=True, strategy_name = None, benchmark_name = None, ic_data = False, turnover_data = False):
    print(turnover)
    if not strategy_name:
        if isinstance(returns, _pd.Series):
            strategy_name = returns.name
    ...
    mtrx = metrics(returns=returns, benchmark=benchmark,
                   rf=rf, rf_rate = rf_rate, display=False, mode='basic',
                   sep=True, internal="True",
                   compounded=compounded)[2:]

    # mtrx = mtrx.iloc[3:,:]
    # mtrx = mtrx.loc[~mtrx.index.isin(['Payoff Ratio', 'Profit Factor', 'Common Sense Ratio', 'CPC Index', 'Tail Ratio', 'Outlier Win Ratio', 'Outlier Loss Ratio'])]
    mtrx.rename(columns = {'Strategy' : strategy_name, 'Benchmark' : benchmark_name}, inplace = True)
    ...
    # random_df = _pd.DataFrame([['a', 'b', 'c', 'd'], 
    #                            [1, 2, 3, 4]], columns=['i', 'j', 'k', 'l']
    #                           )

    # tpl = tpl.replace('{{random_title}}', '<h3>Random table title</h3>')
    # tpl = tpl.replace('{{random_table}}', _html_table(random_df))

    import pdb
    # pdb.set_trace()
    #correlation plot
    if corr:
        tpl = tpl.replace('{{factor_corr}}', _html_table(corr))
    ...
    # plots
    # print(f"'Strategy' : {strategy_name}, 'Benchmark' : {benchmark_name}")
    # import pdb
    # pdb.set_trace()
    figfile = _utils._file_stream()
    # _plots.returns(returns, benchmark, strategy_name = strategy_name, benchmark_name = benchmark_name ,grayscale=grayscale,
    #                figsize=(8, 5), subtitle=False,
    #                savefig={'fname': figfile, 'format': 'svg'},
    #                show=False, ylabel=False, cumulative=compounded)
    ...
    figfile = _utils._file_stream()
    if corr:
        _plots.corr_plot(corr, grayscale=grayscale,
                    figsize=(10, 8), cbar=False,
                    savefig={'fname': figfile, 'format': 'svg'},
                    show=False, ylabel=False)
        
        tpl = tpl.replace('{{corr_plot}}', figfile.getvalue().decode())
    ...
    # _plots.yearly_returns(returns, benchmark, benchmark_name = benchmark_name, strategy_name = strategy_name, grayscale=grayscale,
    #                       figsize=(8, 4), subtitle=False,
    #                       savefig={'fname': figfile, 'format': 'svg'},
    #                       show=False, ylabel=False, compounded=compounded)
    ...
    if turnover:
        _plots.turnover_plot(returns = turnover,grayscale=grayscale,
                    figsize=(8, 5), subtitle=False,
                    savefig={'fname': figfile, 'format': 'svg'},
                    show=False, ylabel=False, cumulative=compounded)
        tpl = tpl.replace('{{turnover_plot}}', figfile.getvalue().decode())

        figfile = _utils._file_stream()
    
    if ic:
        _plots.ic_plot(returns = ic,grayscale=grayscale,
                    figsize=(8, 5), subtitle=False,
                    savefig={'fname': figfile, 'format': 'svg'},
                    show=False, ylabel=False)
        tpl = tpl.replace('{{ic_plot}}', figfile.getvalue().decode())
...
def metrics(returns, benchmark=None, rf=0., rf_rate = 0, display=True,
    ...
    metrics['Return/Risk'] = _stats.sharpe(df, rf_rate)
    metrics['Sortino'] = _stats.sortino(df, rf_rate)
