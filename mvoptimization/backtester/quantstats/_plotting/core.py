...
def plot_returns_bars(returns, benchmark=None,
    ...
    # print(f"plot_returns func : 'Strategy' : {strategy_name}, 'Benchmark' : {benchmark_name}")
    ...
    df = _pd.DataFrame(index=returns.index, data={strategy_name: returns})
    if isinstance(benchmark, _pd.Series):
        df[benchmark_name] = benchmark[benchmark.index.isin(returns.index)]
        df = df[[benchmark_name, strategy_name]]
...
def plot_timeseries2(returns, benchmark=None,
                    title="Returns", strategy_name = None, benchmark_name = None,
                    compound=False, cumulative=True,
    ...
    # print(f"plot_timeseries func : 'Strategy' : {strategy_name}, 'Benchmark' : {benchmark_name}")
    ...
    ax.plot(returns, lw=lw, label=strategy_name, alpha=alpha)
...
def plot_timeseries(returns, benchmark=None,
    ...
    alpha = .25 if grayscale else 1
    ax.plot(returns, lw=lw, label=strategy_name, color=colors[1], alpha=alpha)
...
def plot_ic(returns, resample="M", bins=20,
    ...
    # pdb.set_trace()
    # print(returns)
    _sns.barplot(y = returns.values, x = returns.index,
                color=colors[0],
                ax=ax)
