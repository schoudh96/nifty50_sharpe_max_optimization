...
def turnover_plot(returns,benchmark=None,
            benchmark_name = None, strategy_name = None,
            grayscale=False, figsize=(10, 6),
            fontname='Arial', lw=1.5,
            match_volatility=False, compound=True, cumulative=True,
            resample=None, ylabel="Turnover %",
            subtitle=True, savefig=None, show=True):
    
    print(returns)

    title = 'Strategy Turnover Plot'
    ...
    # print(f"returns func : 'Strategy' : {strategy_name}, 'Benchmark' : {benchmark_name}")

    _core.plot_timeseries2(returns = returns, benchmark = benchmark, title = title,
...
def returns(returns, benchmark=None,
    ...
    _core.plot_timeseries(returns = returns, benchmark = benchmark, title = title,
                          benchmark_name = benchmark_name,
                          strategy_name = strategy_name, 
                          ylabel=ylabel,
                          match_volatility=match_volatility,
...
def yearly_returns(returns, benchmark=None, benchmark_name = None, strategy_name = None,
    ...
    _core.plot_returns_bars(returns, benchmark,
                            benchmark_name = benchmark_name, strategy_name = strategy_name,
                            fontname=fontname,
                            hline=hline,
                            hlw=hlw,
                            hllabel=hllabel,
...
def ic_plot(returns, resample='M', fontname='Arial',
              grayscale=False, figsize=(10, 5), ylabel=True,
              subtitle=True, compounded=True, savefig=None, show=True):
    
    title = f"Mean IC across periods"

    _core.plot_ic(  returns,
                    resample=resample,
                    grayscale=grayscale,
                    fontname=fontname,
                    title=title,
...
def corr_plot(corr, annot_size=10, figsize=(10, 5),
    ...
    cmap = 'gray' if grayscale else 'YlGnBu'
    #'RdYlGn'
    #YlGnBu
    ...
    ax.set_title('   Factor Correlation Plot \n', fontsize=14, y=.995,
                 fontname=fontname, fontweight='bold', color='black')

    # _sns.set(font_scale=.9)
    ax = _sns.heatmap(corr, ax=ax, annot=True, center=0,
    ...
    if ylabel:
        ax.set_ylabel('Factors', fontname=fontname,
                      fontweight='bold', fontsize=12)
        ax.yaxis.set_label_coords(-.1, .5)

    ax.tick_params(colors="#808080")
    _plt.xticks(rotation=90, fontsize=annot_size)
