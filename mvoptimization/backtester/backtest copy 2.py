...
def fetch_bt(holdings, prices, returns, rebal_freq = 1):
    """
    Computes the backtest for given portfolios.
    Either of prices or returns along with holdings should be provided.
    
    :param holdings_df: holdings DataFrame has datewise portfolios
    ...
    """
    rename_columns = ['date', 'secid', 'weight']
    holdings_df.columns = rename_columns
    ...
    if returns:
        ...
        raise Exception('One of either prices or returns should be provided.')
